...
type FormType = "hogar" | "escolar" | "fim";
...
const COPY = {
  spm2: {
    eyebrow: "Operaria Health · OperaHands · SPM-2",
    em: "evaluación",
    sub: "Ingresa los datos y enviamos el link de la encuesta. El informe llega solo a tu correo.",
    info: "/que-es/terapeuta",
  },
  fim: {
    eyebrow: "Operaria Health · OperaHands · FIM",
    em: "independencia funcional",
    sub: "La aplicas tú durante la sesión y ves el resultado al instante. Sin enviar nada.",
    info: "/que-es/fim",
  },
};

export default function HandsToLauncher({ modo = "spm2" }: { modo?: "spm2" | "fim" }) {
  const esFim = modo === "fim";
  const c = esFim ? COPY.fim : COPY.spm2;

  const [formType, setFormType] = useState<FormType>(esFim ? "fim" : "hogar");
  ...
  const isFim = formType === "fim";
  const respLabel = formType === "hogar" ? "Papá, mamá o cuidador/a" : "Profesor/a o educador/a";
  const sujetoLabel = isFim ? "Persona evaluada" : "Niño o niña evaluado/a";
  ...
  const buildLink = () => {
    ...
    if (isFim) return `${origin}/fim/${nameToSlug(toName)}?${params.toString()}`;
  ...
  const submit = async () => {
    setError("");
    if (!childName.trim() || !toName.trim() || (!isFim && !respName.trim())) {
      setError(isFim
        ? "Completa el nombre de la persona evaluada y tu nombre."
        : "Completa los nombres del niño/a, de quien responde y del terapeuta.");
      return;
    }
    const edadNum = parseInt(edad, 10);
    if (!edad.trim() || isNaN(edadNum)) {
      setError("Ingresa la edad (en años).");
      return;
    }
    if (!isFim && (edadNum < 5 || edadNum > 12)) {
    ...
    // FIM: aplicación y respuesta inmediata → el TO abre el formulario y lo aplica.
    if (isFim) {
    ...
    <main className="theme-health min-h-screen bg-offwhite flex flex-col">
      ...
      >
        ...
        <div className="relative z-[2] max-w-xl mx-auto w-full">
          <p className="font-mono text-teal-light text-[11px] uppercase tracking-[0.2em] mb-4">{c.eyebrow}</p>
          <h1 className="font-display font-light text-offwhite leading-[1.05] text-[clamp(34px,7vw,52px)]">
            Nueva <em className="italic text-teal-light">{c.em}</em>
          </h1>
          <p className="font-sans text-offwhite/75 text-[16px] mt-3 max-w-md leading-relaxed">{c.sub}</p>
          <a
            href={c.info}
            target="_blank"
            rel="noreferrer"
          ...
          <>
            {!esFim && (
              <>
                ...
                <p className="font-mono text-muted text-[10px] uppercase tracking-[2px] mb-2 -mt-1">SPM-2 · la responde un tercero</p>
            ...
            <Label>{sujetoLabel}</Label>
            <Input value={childName} onChange={setChildName} placeholder={isFim ? "Nombre de la persona evaluada" : "Nombre del niño o niña"} autoCapitalize="words" />
            <Input
              value={edad}
              onChange={(v) => setEdad(v.replace(/[^0-9]/g, "").slice(0, 2))}
              placeholder={isFim ? "Edad en años" : "Edad en años (5 a 12)"}
              type="number"
              className="mt-3"
            />
            {!isFim && (
            ...
            >
              {isFim ? "Abrir y aplicar la FIM" : status === "sending" ? "Enviando..." : "Enviar invitación"}
            </button>
            <p className="mt-4 text-[13px] text-muted text-center">
              {isFim
                ? "Se abre el formulario para que lo apliques. Al terminar, ves el resultado al instante."
