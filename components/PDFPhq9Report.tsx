...
import {
  PHQ9_ITEM_IDS,
  phq9BandaLabel,
  phq9BandaInterpretacion,
} from "@/lib/phq9";
import { phq9Bloques } from "@/lib/verticals/phq9";
...
function bandaColor(b: Phq9Result["banda"]): string {
  if (b === "minima") return C.green;
  if (b === "leve") return C.yellow;
  if (b === "moderada") return C.orange;
  if (b === "moderadamente_severa") return C.red;
  return C.redDeep;
}
...
const s = StyleSheet.create({
  ...
  alertBox: { backgroundColor: C.redDeep, padding: 12, borderRadius: 4, marginBottom: 14, borderLeft: `4pt solid #fff` },
  alertTitle: { color: "#fff", fontSize: 12, fontWeight: 700, marginBottom: 4, letterSpacing: 0.4 },
  alertText: { color: "#fff", fontSize: 9, lineHeight: 1.5 },
  ...
  itemValor: { width: 110, fontSize: 9, color: C.navy, fontFamily: "Courier", textAlign: "right" },
...
const ITEMS = phq9Bloques[0].preguntas;
...
export function PDFPhq9Report({ pacienteName, edad, fecha, tratanteName, result }: Props) {
  const color = bandaColor(result.banda);
  const bandaLabel = phq9BandaLabel(result.banda);
    ...
    <Document>
      <Page size="LETTER" style={s.page}>
        <View style={s.header} fixed>
          <View>
            <Text style={s.headerTitle}>Resultados PHQ-9 — Tamizaje de Depresión</Text>
            <Text style={s.headerSub}>PATIENT HEALTH QUESTIONNAIRE · 9 ÍTEMS</Text>
          </View>
        ...
        <View style={s.meta}>
          ...
          <View><Text style={s.metaLabel}>Instrumento</Text><Text style={s.metaValue}>PHQ-9 (Kroenke 2001)</Text></View>
        </View>

        {result.ideacionSuicida && (
          <View style={s.alertBox} wrap={false}>
            <Text style={s.alertTitle}>ALERTA — Ítem 9 positivo (ideación de autoagresión)</Text>
            <Text style={s.alertText}>
              El paciente respondió «{result.ideacionSuicidaRespuesta}» al ítem
              «Pensamientos de que estaría mejor muerto/a o de hacerse daño».
              Se sugiere contacto clínico oportuno y evaluación de riesgo. El paciente
              recibió en pantalla los recursos de emergencia disponibles en Chile
              (Salud Responde 600 360 7777, *4141, SAMU 131).
            </Text>
          </View>
        )}
        ...
        <View style={[s.scoreBox, { backgroundColor: color }]}>
          ...
          <Text style={s.scoreNum}>{result.totalScore} / 27</Text>
        ...
        <View style={s.interpretacionBox} wrap={false}>
          <Text style={s.interpretacionText}>{phq9BandaInterpretacion(result.banda)}</Text>
        </View>

        {result.funcionalidad && (
          <View style={s.funcionalBox} wrap={false}>
            <Text style={s.funcionalLabel}>Impacto funcional (ítem 10)</Text>
        ...
        <View>
          {ITEMS.map((p, i) => {
            const id = PHQ9_ITEM_IDS[i];
            const score = result.itemScores[id] ?? 0;
            const alerta = id === "phq9_q9" && score > 0;
            return (
              <View key={id} style={i % 2 === 0 ? s.itemRow : s.itemRowAlt} wrap={false}>
                ...
                <Text style={[s.itemValor, alerta ? { color: C.redDeep, fontWeight: 700 } : {}]}>
                  {score} · {result.ideacionSuicidaRespuesta && id === "phq9_q9" ? result.ideacionSuicidaRespuesta : ""}
                </Text>
              </View>
            );
          })}
        ...
        <View style={s.disclaimer} wrap={false}>
          ...
          <Text style={s.disclaimerText}>
            El PHQ-9 orienta sobre la severidad de síntomas depresivos pero no
            reemplaza la entrevista clínica. Un puntaje en rango moderado o superior
            sugiere evaluación por médico o profesional de salud mental.
          </Text>
